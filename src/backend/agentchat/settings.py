...
async def initialize_app_settings(file_path: str = None):
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            # Docker/环境变量覆盖：优先使用 DATABASE_URL（如 docker-compose 传入）
            if os.environ.get('DATABASE_URL'):
                db_url = os.environ['DATABASE_URL']
                if db_url.startswith('mysql://'):
                    app_settings.mysql['endpoint'] = db_url.replace('mysql://', 'mysql+pymysql://', 1)
                    app_settings.mysql['async_endpoint'] = db_url.replace('mysql://', 'mysql+aiomysql://', 1)
                else:
                    app_settings.mysql['endpoint'] = db_url
                    app_settings.mysql['async_endpoint'] = db_url
                logger.info("MySQL endpoint overridden from DATABASE_URL")
            if os.environ.get('REDIS_URL'):
                app_settings.redis['endpoint'] = os.environ['REDIS_URL']
                logger.info("Redis endpoint overridden from REDIS_URL")
